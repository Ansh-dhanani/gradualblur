...
function generateBlurHTML(config) {
  config = applyPreset(config);
  const { divCount = 6, zones, animated, absolute = true } = config;
  const positions = zones || [config.position || 'top'];
  
  let html = '';
  
  positions.forEach(pos => {
    const suffix = zones ? `-${pos}` : '';
    const positionClass = absolute ? '' : ' gradual-blur-relative';
    html += `<div class="gradual-blur${suffix}${animated === 'fade' ? ' gradual-blur-animated' : ''}${positionClass}">${zones ? '' : '\n'}  <div class="gradual-blur-inner${suffix}">\n`;
    for (let i = 1; i <= divCount; i++) {
